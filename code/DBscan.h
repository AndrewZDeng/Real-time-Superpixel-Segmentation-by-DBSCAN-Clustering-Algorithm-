...
void mergin(int n, unsigned char *R, unsigned char *G, unsigned char *B, int Rows,
            int Cols, unsigned short *label, double *labelsL, double *labelsa, double *labelsb, double *labelsx,
            double *labelsy) {
  int k, bl, m;
...
void
DBscan(unsigned char *R, unsigned char *G, unsigned char *B, int Rows, int Cols, unsigned short *label, int supnumber,
       int &realnumber, int post) {
  int pixel = Rows * Cols;
  supnumber = static_cast<int>(static_cast<double>(supnumber) * 1.1);
  ...
  for (int n = 1; n <= Nc; n++) {
    if (labnumb[n] < supnumber && labnumb[n] > 0) {
      ...
      if (minnum != 0) {
        ...
        labelsR[minnum] =
                (labelsR[minnum] * labnumb[minnum] + labelsR[n] * labnumb[n]) / (labnumb[minnum] + labnumb[n]);
        labelsG[minnum] =
                (labelsG[minnum] * labnumb[minnum] + labelsG[n] * labnumb[n]) / (labnumb[minnum] + labnumb[n]);
        labelsB[minnum] =
                (labelsB[minnum] * labnumb[minnum] + labelsB[n] * labnumb[n]) / (labnumb[minnum] + labnumb[n]);
        labelsx[minnum] =
                (labelsx[minnum] * labnumb[minnum] + labelsx[n] * labnumb[n]) / (labnumb[minnum] + labnumb[n]);
        labelsy[minnum] =
                (labelsy[minnum] * labnumb[minnum] + labelsy[n] * labnumb[n]) / (labnumb[minnum] + labnumb[n]);
